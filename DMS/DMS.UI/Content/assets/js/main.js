...
function applySelect($selector) {
    ...
    $('.ch_vou03led05code').each(function (index, item) {
        ...
        //console.log("test3 : ", values.split(","));
        if (typeof values != undefined) {
            var a1 = values.split(",");
            for (var i = 0; i < a1.length; i++) {
                //console.log(a1[i]);
                //console.log("option[value=" + a1[i] + "]");

                if (a1[i] != "") {
                    $(this).prev().find("option[value=" + a1[i] + "]").attr("selected", "selected");
                }
    ...
    $("select.search-select-ajax").each(function (index, item) {
        console.log($(this).data('ajax-url'));
        $(this).select2({
            ajax: {
...
var Main = function () {
    ...
    var runContainerHeight = function () {
        ...
        var mainNavMenu = mainNavigation.find('.main-navigation-menu');
        //console.log(mainNavMenu);
        if ($windowWidth > 991) {
            var slHeight = mainNavigation.find('.logo-area').outerHeight(true);
            slHeight = isNaN(slHeight) ? 0 : slHeight;
            mainNavMenu.css({
                height: $windowHeight - topBar.outerHeight(true) - slHeight
            });
            $(".navbar-content").css({
        ...
        console.log('horizontal: ', $("#horizontal-menu").length);
        console.log('horizontal: ', mainContent, $windowHeight, topBar.outerHeight(true), footer.outerHeight(true));
        if ($("#horizontal-menu").length) {
            mainContent.css({
    ...
    var limitNumber = function () {
        $(document).on('keydown', '.number, .currency', function (e) {
        ...
        $(document).on('paste', '.number, .currency', function (e) {
    ...
    var limitNumberExt = function () {
        $(document).on('keydown', '.numberExt, .currencyExt', function (e) {
            // Allow: backspace, delete, tab, escape, enter and .
            if ($.inArray(e.keyCode, [46, 8, 9, 27, 13, 110, 190]) !== -1 ||
                // Allow: Ctrl+A, Command+A
                (e.keyCode === 65 && (e.ctrlKey === true || e.metaKey === true)) ||
        ...
        $(document).on('paste', '.numberExt, .currencyExt', function (e) {
            //console.log('paste called : ->', $(this).val());
            $(this).val($(this).val().replace(/\D+/g, ''));
            //onlyNum($(this));
        });
